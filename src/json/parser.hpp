...
struct json_grammar : qi::grammar<Iterator, Json(), ascii::space_type> {
    json_grammar() : json_grammar::base_type(root) {
        ...
        value = (literal | object | array | number | string)[_val = construct<JsonNode>(_1)];

        literal = lit("false")[_val = val(JsonLiteral(JSON_FALSE))] | lit("true")[_val = val(JsonLiteral(JSON_TRUE))] | lit("null")[_val = val(JsonLiteral(JSON_NULL))];

        object = ('{' >> -(member % ',') >> '}')[_val = construct<JsonObject>(_1)];
        member = (string_inner >> ':' >> value)[_val = construct<std::pair<std::string, JsonNode>>(_1, _2)];

        array = ('[' >> -(value % ',') >> ']')[_val = construct<JsonArray>(_1)];

        number = qi::as_string[lexeme[-char_('-') >> +digit >> -frac >> -exp]][_val = construct<JsonNumber>(_1)];
        frac = char_('.') >> +digit;
        ...
        escaped = char_('\\') >> (char_('\\') | char_('"') | char_('n') | char_('b') | char_('f') | char_('r') | char_('t') | (char_('u') >> qi::repeat(1, 4)[ascii::xdigit]));
    }
    ...
    qi::rule<Iterator, std::pair<std::string, JsonNode>(), ascii::space_type> member;
...
Json parse_json(Iterator first, Iterator last) {
    Json json;
    if (!qi::phrase_parse(first, last, json_grammar<Iterator>(), ascii::space, json)) {
