...
class JsonObject {
    public:
        ...
        JsonObject() = default;
        JsonObject(const std::vector<std::pair<std::string, JsonNode>>& members): members(members) {
        }
        // needed by parser
        JsonObject(const boost::optional<std::vector<std::pair<std::string, JsonNode>>>& members) {
            if (members) {
                this->members = boost::get(members);
            }
        }

        bool operator==(const JsonObject&) const = default;
};
...
class JsonNode {
    public:
        ...
        boost::variant<JsonString, JsonNumber, JsonObject, JsonArray, JsonLiteral> inner;
